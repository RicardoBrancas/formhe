...
class ASPSpecGenerator:
    ...
    def __init__(self, instance: Instance, free_vars: int, predicates: list = None) -> None:
        ...
        if predicates is None:
            self.predicates = self.instance.predicates.items()
        else:
    ...
    def generate_type_spec(self) -> TypeSpec:
        ...
        self.Terminal = EnumType('Terminal', OrderedSet(['_', 0, 1]) | self.instance.definitions)
    ...
    def generate_prod_spec(self) -> ProductionSpec:
        ...
        prod_spec.add_func_production('aggregate_term', self.Aggregate,
                                      [(self.Terminal, self.Empty), (self.Terminal, self.PBool), self.PBool,
                                       (self.Terminal, self.Empty)], constant_expr=True)
        ...
        prod_spec.add_func_production('lt', self.Bool, [self.Terminal, self.Terminal], constant_expr=True)
        prod_spec.add_func_production('le', self.Bool, [self.Terminal, self.Terminal], constant_expr=True)
        prod_spec.add_func_production('gt', self.Bool, [self.Terminal, self.Terminal], constant_expr=True)
        prod_spec.add_func_production('ge', self.Bool, [self.Terminal, self.Terminal], constant_expr=True)
    ...
    def generate_pred_spec(self):
        ...
        if not self.instance.config.enable_pool_operator:
        ...
        if not self.instance.config.enable_arithmetic:
            ...
            pred_spec.add_predicate('not_occurs', ['lt'])
            pred_spec.add_predicate('not_occurs', ['le'])
            pred_spec.add_predicate('not_occurs', ['gt'])
            pred_spec.add_predicate('not_occurs', ['ge'])
        ...
        pred_spec.add_predicate('commutative', ['pool'])

        for predicate_name, _ in self.predicates:
            for p in self.prod_spec.get_function_productions():
                if p.name != 'interval' and p.name != 'pool':
                    pred_spec.add_predicate('is_not_parent', [predicate_name, p.name])

        for predicate_name_1, _ in self.predicates:
            for predicate_name_2, _ in self.predicates:
                pred_spec.add_predicate('is_not_grandparent', [predicate_name_1, predicate_name_2])
        ...
        pred_spec.add_predicate('distinct_args', ['lt'])
        pred_spec.add_predicate('distinct_args', ['le'])
        pred_spec.add_predicate('distinct_args', ['gt'])
        pred_spec.add_predicate('distinct_args', ['ge'])
