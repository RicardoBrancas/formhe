...
class ProductionSpec:
    ...
    def get_function_productions(self) -> List[FunctionProduction]:
        """
    ...
    def add_enum_production(self, lhs: EnumType, choice: int, constant_expr: bool = True) -> EnumProduction:
        """
        Create a new enum production. Return the created production.
        Raise `ValueError` if `choice` is out of bound.
        """
        prod = EnumProduction(self._get_next_id(), lhs, choice, constant_expr)
        self._add_production(prod)
        return prod

    def add_param_production(self, lhs: ValueType, index: int, constant_expr: bool = False) -> ParamProduction:
        """
        ...
        if index in self._param_map:
            raise ValueError('Parameter Production with index {} has already been created'.format(index))
        prod = ParamProduction(self._get_next_id(), lhs, index, constant_expr)
        self._param_map[index] = prod
        self._add_production(prod)
        return prod

    def add_func_production(self, name: str, lhs: LeafType, rhs: List[Type], constraints=None,
                            constant_expr=False) -> FunctionProduction:
        """
        ...
        if name in self._func_map:
            raise ValueError('Function Production with name {} has already been created'.format(name))
        prod = FunctionProduction(self._get_next_id(), name, lhs, rhs, constraints, constant_expr)
...
class ProgramSpec:
    ...
    def check_type(ty):
        if isinstance(ty, tuple):
            for t in ty:
                if not isinstance(t, LeafType):
                    raise ValueError('Non-leaf type cannot be used as program input/output: {}'.format(ty))
        elif not isinstance(ty, LeafType):
            raise ValueError('Non-leaf type cannot be used as program input/output: {}'.format(ty))

    def __init__(self, name: str, in_types: List[Type], out_type: Type):
        for ty in in_types:
            self.check_type(ty)
        self.check_type(out_type)
...
class TyrellSpec:
    ...
    def __init__(self, type_spec, prog_spec, prod_spec=ProductionSpec(), pred_spec=PredicateSpec()):
        # Generate all enum productions
        self._add_enum_productions(prod_spec, filter(lambda ty: isinstance(ty, EnumType), type_spec.types()))
        ...
        self.max_children = max(map(lambda p: len(p.rhs), self.productions()))
