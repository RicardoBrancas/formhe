...
r296 <- read_data('296') # default - codegemma 7b 4bits
...
r304 <- read_data('304') # default w/o llm + mutation only repair

source('analysis/plots.R')

times_inverse_cactus(f = r304, filter_expr = expr(!synthetic))
times_inverse_cactus(f = r304, filter_expr = expr(!synthetic), get_data = T)

times_inverse_cactus(f = r296, filter_expr = expr(!synthetic))
times_inverse_cactus(f = r296, filter_expr = expr(!synthetic), get_data = T)
...
plot_pdf('repair_abduction', 1 * textwidth, .4 * textwidth, repair_abduction_plot)
...
cat("Longest repair answer with impl choosing:", (r296 %>% filter(solved) %>% summarise(n = max(real)))[[1]])
cat("Longest repair answer without impl choosing:", (r295 %>% filter(solved) %>% summarise(n = max(real)))[[1]])

cat("Total instances with feedback:", (r296 %>% filter(!synthetic) %>%
  ...
  tally() / r296 %>% filter(!synthetic) %>%
  filter(feedback_type != 'Solution OK' &
           feedback_type != 'Parsing Error' &
           feedback_type != 'Grounding Error') %>%
  tally())[[1]])

cat("Total instances with at least one line correctly identified:", (r296 %>% filter(!synthetic) %>%
  filter(fault.identified == "Yes (no incorrect lines)" |
           fault.identified == "Yes (first MCS)" |
           fault.identified == "Subset (first MCS)" |
           fault.identified == "Superset (first MCS)" |
...
cat("Total instances with at least one line correctly identified & repair:", (r296 %>% filter(!synthetic) %>%
  filter((fault.identified == "Yes (no incorrect lines)" |
           fault.identified == "Yes (first MCS)" |
           fault.identified == "Subset (first MCS)" |
           fault.identified == "Superset (first MCS)" |
...
cat("Total instances with repair but wrong FL:", (r296 %>% filter(!synthetic) %>%
  filter(!(fault.identified == "Yes (no incorrect lines)" |
           fault.identified == "Yes (first MCS)" |
           fault.identified == "Subset (first MCS)" |
           fault.identified == "Superset (first MCS)" |
